...
class LaporanAsharSiswa extends Page implements HasTable
{
    use InteractsWithTable;
    
    protected static ?string $navigationIcon = 'heroicon-o-document-text';

    protected static string $view = 'filament.pages.laporan-ashar-siswa';
    ...
    public function table(Table $table): Table
    {
        ...
        if(auth()->user()->level === 'guru' || auth()->user()->level === 'kepsek') {
            $queryAshar = \App\Models\Siswa::query()
                            ->whereHas('kelas', fn (Builder $query) => $query->where('jenjang',auth()->user()->guru->jenjang))
                            ->withCount(['absenSiswa as hadir' => fn (Builder $query) => $query->where('status', 'hadir')->where('tipe_absen', 'ashar')->where('semester_id', $semesterId)])
                            // Menghitung jumlah 'izin'
                            ->withCount(['absenSiswa as izin' => fn (Builder $query) => $query->where('status', 'izin')->where('tipe_absen', 'ashar')->where('semester_id', $semesterId)])
                            // Menghitung jumlah 'sakit'
                            ->withCount(['absenSiswa as sakit' => fn (Builder $query) => $query->where('status', 'sakit')->where('tipe_absen', 'ashar')->where('semester_id', $semesterId)])
                            // Menghitung jumlah 'alpha'
                            ->withCount(['absenSiswa as alpha' => fn (Builder $query) => $query->where('status', 'alpha')->where('tipe_absen', 'ashar')->where('semester_id', $semesterId)]);
        } else {
            $queryAshar = \App\Models\Siswa::query()
                        ->withCount(['absenSiswa as hadir' => fn (Builder $query) => $query->where('status', 'hadir')->where('tipe_absen', 'ashar')->where('semester_id', $semesterId)])
                        // Menghitung jumlah 'izin'
